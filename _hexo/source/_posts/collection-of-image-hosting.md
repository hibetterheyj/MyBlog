...
> 本文原则上不收集个人用 chevereto 搭建的公共图床。因为个人搭建的公共服务稳定性是没有太大保证的。
...
> 你如果是坚持抵制奇虎 360，你大可以不用这个图床。奇虎的这个图床并不公开，没有像微博图床一样被滥用，所以无需担心有过于严格的审查；图片托管在奇虎图床的风险相比微博图床是要低的。
...
> 太大的图片的话压图有一点严重，所以不是太推荐使用。我认识的几个站有在用这个图床。
> 这个图床原本还有一个接口，就是淘宝的淘盘，传完图片后可以获取图片外链，后来在云盘大清洗中被关停了。
